...
using SquareType = std::conditional_t<board_size <= 15, uint8_t, uint16_t>;
...
template <size_t board_size> bool is_north_edge(size_t idx) {
  size_t row = (idx / board_size);
  return row == 0;
}

template <size_t board_size> bool is_south_edge(size_t idx) {
  size_t row = (idx / board_size);
  return row == (board_size - 1);
}

template <size_t board_size> bool is_west_edge(size_t idx) {
  size_t col = (idx % board_size);
  return col == 0;
}

template <size_t board_size> bool is_east_edge(size_t idx) {
  size_t col = (idx % board_size);
  return col == (board_size - 1);
}

template <size_t board_size> bool is_on_edge(size_t idx) {
  return is_east_edge<board_size>(idx) | is_west_edge<board_size>(idx) |
             is_south_edge<board_size>(idx) ||
         is_north_edge<board_size>(idx);
}

template <size_t board_size> constexpr uint64_t get_board_mask() {
